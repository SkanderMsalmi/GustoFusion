...
const UpdateChef = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8088/api/employe/api/chef/${params.id}`)
      .then((response) => {
        setBadge(response.data.badge);
        setFirstName(response.data.prenom);
        setLastName(response.data.nom);
        setSalary(response.data.salaire);
        setSpecialty(response.data.specialite);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const handleSubmit = (event) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      axios
        .put(`http://localhost:8088/api/employe/api/chef/${params.id}`, chef)
        .then((response) => {
          console.log(response);
          window.location.href = "/admin/employees/chefs"; // redirect to /chefs
        })
        .catch((error) => {
