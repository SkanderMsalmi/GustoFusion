...
const UpdateSousChef = (props) => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8083/api/sous-chef/${params.id}`)
            .then(response => {
                const sousChef = response.data;
                setBadge(sousChef.badge);
                setPrenom(sousChef.prenom);
                setNom(sousChef.nom);
                setSalaire(sousChef.salaire);
                setTypeService(sousChef.typeService);
                setChefId(sousChef.chef.id);
            })
            .catch(error => {
                console.log(error);
            });
    ...
    const handleSubmit = (event) => {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            const sousChef = { badge:Number(badge), prenom:prenom, nom:nom, salaire:Number(salaire), typeService:typeService, chef:chef };
            axios.put(`http://localhost:8083/api/sous-chef/${params.id}`, sousChef)
              .then(response => {
        ...
        <div className="container" style={{color: "black"}}>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                    ...
                    {errors.typeService && <div className="invalid-feedback">{errors.typeService}</div>}
                </div>
                <div className="form-group">
                    ...
                    {errors.chefId && <div className="invalid-feedback">{errors.chefId}</div>}
                </div>
